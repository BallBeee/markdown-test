...
const string = { class: "const" as const };
abstract class Yield {
  protected abstract get [string.class](): type;
  public async *yield(type: type) {
    yield await import(`${string.class}`);
  }
}
```
...
DOM 结构、各类标记语言。需要重点关注嵌套在标签内的 Attributes 和字符串高亮是否清晰分离。

```html
<!DOCTYPE html>
<style class="style" style="content: '<style>';">/*<style>*/</style>
<div data-script="<script>" class="class class-name" hidden>
  <custom-tag title="title">
    Text node with entities: &amp; &lt;div&gt; "quotes"
    <script>let div = "<div title='title'></div>";</script>
  </custom-tag>
</div>
```
